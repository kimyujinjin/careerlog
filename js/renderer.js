...
function renderProjectItem(p) {
  ...
  return `
    <div class="r-proj-item">
      ...
      ${p.background ? `<div class="r-exp-proj__section"><p class="r-exp-proj__section-title">[배경 및 목적]</p><ul class="r-exp-proj__bullets"><li>${esc(p.background).replace(/\n/g,'<br>')}</li></ul></div>` : ''}
      ${p.mainTasks && p.mainTasks.filter(Boolean).length ? `<div class="r-exp-proj__section"><p class="r-exp-proj__section-title">[주요 업무]</p><ul class="r-exp-proj__bullets">${p.mainTasks.filter(Boolean).map(t => `<li>${esc(t)}</li>`).join('')}</ul></div>` : ''}
      ${p.achievements && p.achievements.filter(Boolean).length ? `<div class="r-exp-proj__section"><p class="r-exp-proj__section-title">[핵심 성과]</p><ul class="r-exp-proj__bullets">${p.achievements.filter(Boolean).map(a => `<li>${esc(a)}</li>`).join('')}</ul></div>` : ''}
